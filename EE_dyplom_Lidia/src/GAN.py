...
class SystemGAN:
    ...
    def wczytaj_dane(self, nazwa_pliku):
        ...
        try:
            ...
            if df.shape[1] < 3:
                raise ValueError(f"Plik {nazwa_pliku} musi miec co najmniej 3 kolumny.")
    ...
    def generuj(self):
        ...
        plt.plot(t_hist, hist_inv[:, 0], color='green', linewidth=1.2, label='Historia V1')
        plt.plot(t_hist, hist_inv[:, 1], color='blue', linewidth=1.2, linestyle='--', label='Historia V2')
        plt.plot(t_pred, preds_inv[:, 0], color='purple', linewidth=1.0, label='Prognoza V1')
        plt.plot(t_pred, preds_inv[:, 1], color='red', linewidth=1.0, linestyle='--', label='Prognoza V2')

        plt.title("Przebieg czasowy sygnalow")
        plt.xlabel("Czas [s]")
        plt.ylabel("Amplituda")

        limit_krokow_wykresu = 300 
        max_czas_na_wykresie = (self.seq_len + limit_krokow_wykresu) * self.timestep
        plt.xlim([0, max_czas_na_wykresie])

        plt.legend(loc='upper right', framealpha=1.0, edgecolor='black')
        plt.grid(True, linestyle=':', color='gray', alpha=0.7) 
        plt.tight_layout()
        plt.savefig(os.path.join(self.folder_name, f"Sygnal_{ts}.png"), dpi=300)
        plt.close()
...
class AnalizaChaosu:
    def __init__(self, folder_name="Wyniki_GAN"):
        ...
        self.styl_ramki = dict(boxstyle='square,pad=0.3', facecolor='white', edgecolor='black')
        if not os.path.exists(self.folder_name):
            os.makedirs(self.folder_name)
    ...
    def przeprowadz_pelna_analize(self, wygenerowany_sygnal, dt_eff, ts):
        self.analiza_entropii(wygenerowany_sygnal, ts)
        self.analiza_widma_mocy(wygenerowany_sygnal, dt_eff, ts)
    ...
    def rysuj_atraktor(self, generowany, ts):
        ...
        plt.plot(generowany[:, 0], generowany[:, 1], color='purple', lw=0.5)
        plt.title("Atraktor")
        plt.xlabel("V1")
        plt.ylabel("V2")
        plt.grid(True, linestyle=':', color='black', alpha=0.7)
        plt.tight_layout()
        plt.savefig(os.path.join(self.folder_name, f"Atraktor_{ts}.png"), dpi=300)
        plt.close()

    def analizuj_lle_gan(self, generowany, ts):
        print("Obliczanie najwiekszego wykladnika Lapunowa oraz Entropii KS...")
        ...
        entropia_ks = mle if mle > 0 else 0.0

        plt.figure(figsize=(8, 5))
        
        plt.plot(divergence, color='black', lw=1.2, label='Dywergencja trajektorii')
        plt.plot(prosta_x, prosta_y, color='red', linestyle='--', lw=1.5, label='Dopasowanie liniowe (LLE)')

        plt.title("Najwiekszy Wykladnik Lapunowa")
        plt.xlabel("Kroki czasowe")
        plt.ylabel("Logarytm odleglosci")
        plt.grid(True, linestyle=':', color='gray', alpha=0.7)
        plt.legend(loc='upper left', edgecolor='blue')

        tekst_wyniku = f"LLE: {mle:.4f}\nEntropia KS: {entropia_ks:.4f}"
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.folder_name, f"Lapunowa_{ts}.png"), dpi=300)
        plt.close()

    def licz_znormalizowana_entropie(self, fragment, liczba_koszy):
    ...
    def analiza_entropii(self, generowany, ts):
        print("Analiza entropii Shannona...")
        rozmiar_okna = 100
        skok = 50
        liczba_koszy = 20
        ...
        for i in range(0, len(generowany) - rozmiar_okna + 1, skok):
            ...
            entropia_gen.append(self.licz_znormalizowana_entropie(frag_gen, liczba_koszy))

        srednia_entropia = np.mean(entropia_gen)

        fig_ent, ax_ent = plt.subplots(figsize=(8, 5))
        ax_ent.plot(entropia_gen, color='blue', linewidth=1.2)
        ax_ent.set_title("Znormalizowana Entropia Shannona")
        ax_ent.set_xlabel("Numer okna pomiarowego")
        ax_ent.set_ylabel("Entropia [-]")
        ax_ent.grid(True, linestyle=':', color='gray', alpha=0.7)

        tekst_ent = f"Srednia entropia: {srednia_entropia:.4f}"
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.folder_name, f"Entropia_Shannona_{ts}.png"), dpi=300)
        plt.close()

    def analiza_widma_mocy(self, generowany, dt_eff, ts):
        print("Obliczanie widma mocy...")
        sygnal_1d = generowany[:, 0]

        widmo = np.fft.fft(sygnal_1d)
        czestotliwosci = np.fft.fftfreq(len(sygnal_1d), d=dt_eff)

        polowa = len(sygnal_1d) // 2
        moc = np.abs(widmo[:polowa])**2
        freqs = czestotliwosci[:polowa]

        fig_widmo, ax_widmo = plt.subplots(figsize=(8, 5))
        ax_widmo.semilogy(freqs, moc, color='blue', linewidth=1.0)
        ax_widmo.set_title("Widmo Mocy")
        ax_widmo.set_xlabel("Czestotliwosc [Hz]")
        ax_widmo.set_ylabel("Gestosc widmowa mocy")
        ax_widmo.grid(True, linestyle=':', color='grey', alpha=0.7)

        plt.tight_layout()
        plt.savefig(os.path.join(self.folder_name, f"Widmo_mocy_{ts}.png"), dpi=300)
        plt.close()
